...
const norm=r=>r.status==="Готово"?"Готово":r.status==="На проверке"?"На проверке":r.stage==="Референсы"?"Превиз-кадры":r.stage==="Видео"?"Генерация":ST.includes(r.stage)?r.stage:(r.status==="В работе"?"Сценарий":"Идея");
...
function productionStageDone(r,k){
  ...
  if(k==="Превиз-кадры")return !!(r.previsResult?.completed&&(r.previsFrames||[]).length);
...
function stageDone(r,stage){
  ...
  if(stage==="Превиз-кадры")return !!(r.previsResult?.completed&&(r.previsFrames||[]).length);
...
function stageReportHtml(r,stage){
  ...
  if(stage==="Идея"){
    ...
    const total=Number(r.previsPlan?.totalFrames)||((board.length||5)*3);
    const grouped=[...new Set(frames.map(x=>Number(x.scene)).filter(Boolean))];
    body='<div class="previs-head"><div><b>'+frames.length+' / '+total+' кадров</b><p>3 кадра на сцену: начало → развитие → финал. Исходные фото используются только для identity, дальше приоритет у уже сгенерированных кадров.</p></div><span class="status '+(r.previsResult?.completed?'done':'work')+'">'+(r.previsResult?.completed?'Готово':'Генерируется')+'</span></div>'+
      (frames.length?'<div class="previs-scenes">'+grouped.map(scene=>'<section class="previs-scene"><div class="previs-scene-title"><b>Сцена '+scene+'</b><small>'+frames.filter(x=>Number(x.scene)===scene).length+' кадра</small></div><div class="previs-grid">'+frames.filter(x=>Number(x.scene)===scene).sort((a,b)=>Number(a.frame)-Number(b.frame)).map(x=>'<article class="previs-card"><div class="previs-image">'+(x.url?'<img src="'+esc(x.url)+'" alt="Превиз сцены '+scene+'">':'<div class="empty compact-empty">Генерация…</div>')+'</div><div class="previs-card-copy"><b>'+esc((x.frameType||"frame").toUpperCase())+' · кадр '+esc(x.frame||"")+'</b><small>'+esc(x.timecode||"")+'</small><p>'+esc(x.goal||x.action||"")+'</p><div class="previs-ref-mode">'+esc(x.referenceMode||"anchor-led")+'</div><details><summary>Cinematic prompt</summary><p>'+esc(x.imagePromptEn||x.imagePromptRu||"—")+'</p><small>Continuity</small><p>'+esc(x.continuityNotes||"—")+'</p></details></div></article>').join("")+'</div></section>').join("")+'</div>':'<div class="empty compact-empty">'+esc(r.previsError||"Кадры ещё не сгенерированы.")+'</div>');
    ...
    body=r.montageResult?.url?'<div class="generated-video-panel"><video controls playsinline preload="metadata" src="'+esc(r.montageResult.url)+'"></video><p class="artifact-note">Длительность: '+esc(r.montageResult.durationSeconds||"—")+' сек</p></div>':'<div class="empty compact-empty">Финальный монтаж ещё не готов.</div>';
