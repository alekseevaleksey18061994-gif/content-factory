...
function scenes(r){
  ...
  return board.map((sc,i)=>{
    ...
    const route=r.sceneRouting?.[n]||null,routeLabel=route?(route.provider==="seedance"?"Seedance 2.0":"Runway"):(r.modelMode||"Авто");
...
function stageReportHtml(r,stage){
  ...
  if(stage==="Идея"){
    ...
    body='<div class="idea-master-report">'+
      ...
      (hookVariants.length?'<div class="idea-alt-wrap"><div class="idea-alt-head"><div><h3>🔥 Hook Lab</h3><p>5 разных первых 3 секунд до дорогой генерации.</p></div><span>выбран №'+esc(String(hookLab.selectedIndex||1))+'</span></div><div class="idea-alt-list">'+hookVariants.map((h,i)=>'<article class="'+((i+1)===Number(hookLab.selectedIndex||1)?"selected":"")+'"><span>'+String(i+1)+'</span><div class="idea-alt-copy"><b>'+esc(h.type||"Hook")+'</b><small>'+esc(h.firstFrame||"")+'</small><p>'+esc([h.action,h.line,h.audio,h.openLoop].filter(Boolean).join(" · "))+'</p><small>'+esc(h.why||"")+'</small></div></article>').join("")+'</div><div class="idea-why"><small>Почему выбран этот хук</small><p>'+esc(hookLab.selectionReason||"—")+'</p></div></div>':'')+
...
window.modelScene=async(id,n)=>{const r=runs.find(x=>x.id===id),current=r?.sceneModels?.[n]||"Авто";const t=prompt("Модель для сцены "+n+":\nАвто — AI Router\nRunway — простая сцена\nSeedance 2.0 — сложная сцена",current);if(!t)return;try{await runServerAction({runId:id,action:"set_scene_model",scene:n,model:t})}catch(e){alert(String(e?.message||e))}};
