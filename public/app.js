...
+usd.toFixed(4)+'</em>':'')+'</div>'
  }).join("");

  const billing=settings.providerBilling&&typeof settings.providerBilling==="object"?settings.providerBilling:{};
  const funding=all.filter(e=>["subscription","topup"].includes(String(e.category||"")));
  const fundingHtml=funding.slice().reverse().map(e=>{
    const rub=expenseRubValue(e);
    return '<div class="billing-item"><div><b>'+esc(e.provider||"Сервис")+'</b><small>'+esc(e.description||"")+'</small></div><div><strong>'+fmtMoney(rub)+' ₽</strong>'+(e.amountUsd?'<small>
...
+Number(e.amountUsd).toFixed(2)+(e.recurring==="monthly"?" / мес.":"")+'</small>':'')+'</div></div>';
  }).join("");
  const balanceHtml=[
    billing.higgsfield?'<div class="billing-balance"><b>Higgsfield</b><span>'+esc(billing.higgsfield.plan||"")+' · '+esc(billing.higgsfield.credits||0)+' кредитов</span></div>':'',
    billing.runway?'<div class="billing-balance"><b>Runway</b><span>'+esc(billing.runway.plan||"")+' · '+esc(billing.runway.totalCredits||0)+' кредитов ('+esc(billing.runway.planCredits||0)+' тариф + '+esc(billing.runway.purchasedCredits||0)+' куплено)</span></div>':''
  ].filter(Boolean).join("");
  if($("#billingSummary"))$("#billingSummary").innerHTML=(fundingHtml||'<div class="empty compact-empty">Подписок и пополнений с известной суммой пока нет.</div>')+(balanceHtml?'<div class="billing-balances">'+balanceHtml+'</div>':'');
