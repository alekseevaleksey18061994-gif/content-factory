...
$("#dashboardStats").innerHTML=stat("⚙","В производстве",active)+stat("◷","На проверке",review,"blue")+stat("✓","Готово",ready)+stat("!","Ошибки",errors,"warn")+stat("➤","Опубликовано",pub,"purple")+stat("₽","Расходы",cost+" ₽");
...
function renderRunDetail(){
  ...
  const generatedVideo=r.outputUrl?'<section class="panel generated-video-panel"><div class="panel-title"><div><span class="mini-icon">▶</span><h2>Видео Higgsfield</h2><p>Результат автоматической генерации</p></div><span class="status done">Готово</span></div><video controls playsinline preload="metadata" style="width:100%;max-height:620px;border-radius:16px;background:#000" src="'+esc(r.outputUrl)+'"></video><div class="catalog-actions"><a class="secondary" href="'+esc(r.outputUrl)+'" target="_blank" rel="noopener">Открыть оригинал ↗</a></div></section>':'';
  $("#runDetailBody").innerHTML=
    '<section class="panel"><div class="section-head" style="margin:0"><div><span class="kicker">РОЛИК</span><h1>'+esc(pname(r))+(r.variant?' · вариант '+r.variant:'')+'</h1><p>'+esc(r.style||'')+' · '+esc(r.duration||'')+' · 9:16 · '+esc((r.platforms||[]).join(' / '))+'</p></div><span class="status '+scl(r.status)+'">'+esc(r.status||'В работе')+'</span></div><div class="progress" style="height:10px;margin-top:18px"><i style="width:'+pct(r)+'%"></i></div><div class="campaign-budget"><span>Прогресс: '+pct(r)+'%</span><span>Режим: '+(r.mode==="manual"?"Ручной":"Автопилот")+'</span></div></section>'+
...
$("#launch").onclick=async()=>{
  ...
  const base={batchId:uid("batch"),productId:$("#productSelect").value,productName:chosenProduct?.name||"Товар",productUtp:chosenProduct?.utp||"",productRules:chosenProduct?.rules||"",productMedia,campaignId:$("#campaignSelect").value||null,characterId:chosenCharacter?.id||null,character:characterPayload,avatarReferences:characterPayload?.media||[],brief:$("#brief").value.trim(),style:$("#style").value,duration:$("#duration").value,count:$("#count").value,format:$("#format").value,platforms,mode:createMode,modelMode:$("#modelMode").value,budget:Number($("#runBudget").value)||500,maxAttempts:Number($("#runAttempts").value)||3,created:now()};
