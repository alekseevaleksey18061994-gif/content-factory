...
const APP_VERSION=(()=>{try{return String(JSON.parse(fs.readFileSync(path.join(__dirname,'package.json'),'utf8')).version||'dev')}catch{return 'dev'}})();
...
const AUTH_REGISTRY_CACHE_MS=30000;
let usersRegistryCache=null;
let usersRegistryCacheAt=0;
let accountsRegistryCache=null;
let accountsRegistryCacheAt=0;
...
async function ensureUsersRegistry(){
  if(usersRegistryCache&&(Date.now()-usersRegistryCacheAt)<AUTH_REGISTRY_CACHE_MS){
    return cloneRegistry(usersRegistryCache);
  }
  const row=await readStateRow(USERS_REGISTRY_ID);
  if(row.data?.users){
    usersRegistryCache=cloneRegistry(row.data);
    usersRegistryCacheAt=Date.now();
    return cloneRegistry(row.data);
  }
...
async function ensureAccountsRegistry(){
  if(accountsRegistryCache&&(Date.now()-accountsRegistryCacheAt)<AUTH_REGISTRY_CACHE_MS){
    return cloneRegistry(accountsRegistryCache);
  }
  const row=await readStateRow(ACCOUNTS_REGISTRY_ID);
  if(row.data?.accounts?.length){
    accountsRegistryCache=cloneRegistry(row.data);
...
async function generatePrevisPlan(payload,accountId,feedback=''){
  ...
  const model=openAIStageModel('previs-plan');
  const r=await fetch('https://api.openai.com/v1/responses',{
    method:'POST',
    headers:{authorization:'Bearer '+process.env.OPENAI_API_KEY,'content-type':'application/json'},
    body:JSON.stringify({model,input:master,reasoning:{effort:'low'},max_output_tokens:9000})
  });
  const raw=await r.text();let data;try{data=raw?JSON.parse(raw):{}}catch{data={raw}}
...
async function generateOpenAIPrevisImage(accountId,run,frame,referenceUrls=[]){
  ...
  const model=openAIStageModel('previs-image');
...
async function buildSceneRouting(run,accountId){
  ...
  try{
    const model=openAIStageModel('scene-router');
    const r=await fetch('https://api.openai.com/v1/responses',{
      method:'POST',
      headers:{authorization:'Bearer '+process.env.OPENAI_API_KEY,'content-type':'application/json'},
      body:JSON.stringify({
...
async function callOpenAIChat(message,history=[],accountId=DEFAULT_ACCOUNT_ID,attachments=[]){
  ...
  const usedModel=data?.model||openAIStageModel('chat');
  ...
  return {
    ...
    model:data?.model||openAIStageModel('chat')
  };
}
...
async function analyzeReferenceMaterial(opts){
  ...
  const model=openAIStageModel('analysis');
  const r=await fetch('https://api.openai.com/v1/responses',{
    method:'POST',
    headers:{authorization:'Bearer '+process.env.OPENAI_API_KEY,'content-type':'application/json'},
    body:JSON.stringify({
      ...
      reasoning:{effort:'medium'},max_output_tokens:10000,
